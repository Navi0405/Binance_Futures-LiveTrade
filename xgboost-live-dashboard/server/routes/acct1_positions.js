...
const client = Binance({
  apiKey: process.env.acct1_key,
  apiSecret: process.env.acct1_secret,
});

async function acct1_position_data() {
  try {
...
router.get('/', async (req, res) => {
  try {
    // Call the acct1_position_data function to get the data
    const openPositions = await acct1_position_data();
...
module.exports.acct1_position_data = acct1_position_data; // For standalone function
