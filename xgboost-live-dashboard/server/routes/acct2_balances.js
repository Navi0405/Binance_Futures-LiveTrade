...
const acct2_secret = process.env.acct2_secret;

const acct2_init_balance = 2023.909;

// Helper function to sign a query
function signQuery(query) {
  return crypto.createHmac('sha256', acct2_secret).update(query).digest('hex');
}
...
async function getFuturesAccountDetails() {
  ...
  try {
    ...
    const returnValue = (parseFloat((usdtBalance.marginBalance - acct2_init_balance)/acct2_init_balance)*100).toFixed(2);
...
async function acct2_balance_data() {
...
router.get('/', async (req, res) => {
  try {
    const { available, marginalBalance, totalWalletBalance, totalUnrealizedPnL, returnValue } = await acct2_balance_data();
...
module.exports.acct2_balance_data = acct2_balance_data; // For standalone function
...
// module.exports = {router, acct2_balance_data};
